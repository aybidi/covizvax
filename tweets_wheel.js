...
var start, end, topics, state, senti;
...
var graph,
  ...
  var allGroup = ["Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6",
  "Topic 7", "Topic 8", "Topic 9", "Topic 10", "All Topics"];
  var sentiGroup = ["Positive", "Negative", "Neutral", "All Sentiment"]
  var stateGroup = ['All States','Alabama','Alaska','American Samoa','Arizona','Arkansas','California','Colorado','Connecticut','Delaware','District of Columbia','Federated States of Micronesia','Florida','Georgia','Guam','Hawaii','Idaho','Illinois','Indiana','Iowa','Kansas','Kentucky','Louisiana','Maine','Marshall Islands','Maryland','Massachusetts','Michigan','Minnesota','Mississippi','Missouri','Montana','Nebraska','Nevada','New Hampshire','New Jersey','New Mexico','New York','North Carolina','North Dakota','Northern Mariana Islands','Ohio','Oklahoma','Oregon','Palau','Pennsylvania','Puerto Rico','Rhode Island','South Carolina','South Dakota','Tennessee','Texas','Utah','Vermont','Virgin Island','Virginia','Washington','West Virginia','Wisconsin','Wyoming'];
...
function jquery(){
  ...
  d3.select("#infoPanel_Data_name").transition().duration(10/2)
    ...
    .style("opacity", 1)
  $('#infoPanel_Data_name').text(node_for_text.text).css('display','')
  $('#restaurant_link_url').attr('href',node_for_text.url)
  $('#tweet_link').css('display','block')
  d3.select("#price_tier").transition().duration(10/2)
    .style("opacity", 0)
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#price_tier').text(node_for_text.retweets + " retweets").css('display','')

  d3.select("#likes").transition().duration(10/2)
    .style("opacity", 0)
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#likes').text(node_for_text.favorites+ " likes").css('display','')
  ...
  d3.select("#infoPanel_Location").transition().duration(10/2)
    ...
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_Location').text("Address:   "+node_for_text.user_location+"").css('visibility','visible')

  d3.select("#infoPanel_Category").transition().duration(10/2)
    .style("opacity", 0)
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_Category').text("Date:   "+node_for_text.date).css('visibility','visible')
  ...
  d3.select("#infoPanel_LastInspect").transition().duration(10/2)
    ...
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_LastInspect').text("Topic:   "+node_for_text.topic_name).css('visibility','visible')
  /*****************************************************************
  * overflow dark_side;
  *
  *****************************************************************/
  ...
  *****************************************************************/
  ...
  if(Number.isInteger(node_for_text.rating)){pdl = "15px" }else{ pdl = "12px"};
      d3.select("#rating_score_text").text(node_for_text.user_followers + " / 10")
      ...
      d3.select('#rating_score_block')
        .style('background-color',color_for_rating(node_for_text.favorites))
  /*****************************************************************
  * overflow rating;
  *
  * positive
    ...
    $('#positive_button').click(function(){
  	...
  	if(tick_increment > 20){
        ...
        // style: 'mapbox://styles/mapbox/satellite-v9',
			  center: [-73.997468948, 40.7141835273],
			  zoom: 6,
			  pitch: 45,
			  bearing: -17.6,
			  container: 'mapbox'
	...
	if(!map_toggle){
  	...
  	if(map_ready){
  		map.getSource('point').setData({
  			"type": "Point",
  			"coordinates": [node_for_text.longitude, node_for_text.latitude]});
  		map.flyTo({
  			center: [node_for_text.longitude,node_for_text.latitude],
  			zoom:3});
  	}
	}
	/********************************* Map end *********************************/
...
function jquery_vio(){
  ...
  d3.select("#infoPanel_vio_code").transition().duration(10/2)
    ...
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_vio_code').text(code_correspond_description[node_for_violation]).css('display' , "inline")
  d3.select("#infoPanel_vio_des").transition().duration(10/2)
    .style("opacity", 0)
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_vio_des').text('').css('display','inline');
  d3.select("#infoPanel_vio_analyze").transition().duration(10/2)
    .style("opacity", 0)
    .transition().duration(1000/2)
    .style("opacity", 1)
  $('#infoPanel_vio_analyze').html("Words: " + topic_words[node_for_violation] + "<br> <br>Total number of tweets that fall under this topic:").css('display','inline');
...
function drawNode_button_last_inspect(d) {
  ...
  if(d.gr == 'r' ){
    if(data[d.id] !== undefined){
      ...
      // TODO: no property named violation
...
// TODO: color definition
function drawNode_tweet_A(d) {
  if(d.gr == 'r' ){
    if(d.gd < -0.33){
...
function drawNode_tweet_B(d) {
  if(d.gr == 'r' ){
    if(d.gd >= -0.33 && d.gd <= 0.33){
      context.moveTo(d.x + 3.3, d.y);
      context.arc(d.x, d.y, 3.3, 0, 2 * Math.PI);
    }
  }
...
/******************************* slide stuff ***************************************/
    ...
    *****************************************************************/
    d3.queue()
      // some changes in the file path, I have some issues with loading data
      .defer(d3.json, 'node_link_data_s.json')
      .defer(d3.json, 'tweets_topics.json')
      .defer(d3.json, 'topic_description.json')
      .defer(d3.json, "topic_words.json")
      .await(function(err, _graph, _data, _code_correspond_description, _topic_words){ if(err) throw err;
...
/** This is the function for filtering nodes and links in our graph
 *  , filter data to everything that comes after
 */
function parseData() {
    // convert date from date range selector to JS Date object
    ...
    // define a variable for filtered graph
    graph = {}

    // grab a copy of the original graph json dataset containing nodes in one
    // variable and links in another
    var nodes_g = original_graph.nodes;
    var links_g = original_graph.links;

    var setNodes;

    // define variables to store filtered nodes and links and also for looping 
    // through the original graph
    var nodesArray = []
    var linksArray = []
    var ind; // it is used to keep track of the nodes given the current user filter
    var top;
    var i;
    var senti_start = -1, senti_end = 1.01; // default range with all sentiments

    // if user selects all topic, include topic name in ind array, otherwise just
    // include topic selected
    if (topics == "All Topics") {
      ind = ["Topic 1", "Topic 2", "Topic 3", "Topic 4", "Topic 5", "Topic 6",
    "Topic 7", "Topic 8", "Topic 9", "Topic 0", "Topic 10"];
    }
    else {
      ind = [topics];
    }

    // if user selects all states, include all state in state_ind array, otherwise
    // just include state selected
    if (state == "All States") {
      state_ind = ['Alabama','Alaska','American Samoa','Arizona','Arkansas','California','Colorado','Connecticut','Delaware','District of Columbia','Federated States of Micronesia','Florida','Georgia','Guam','Hawaii','Idaho','Illinois','Indiana','Iowa','Kansas','Kentucky','Louisiana','Maine','Marshall Islands','Maryland','Massachusetts','Michigan','Minnesota','Mississippi','Missouri','Montana','Nebraska','Nevada','New Hampshire','New Jersey','New Mexico','New York','North Carolina','North Dakota','Northern Mariana Islands','Ohio','Oklahoma','Oregon','Palau','Pennsylvania','Puerto Rico','Rhode Island','South Carolina','South Dakota','Tennessee','Texas','Utah','Vermont','Virgin Island','Virginia','Washington','West Virginia','Wisconsin','Wyoming'];
    }
    else {
      state_ind = [state]
    }
    ...
    // define range based on user sentiment selection
    if (senti == "Positive") {
      senti_start = 0.33;
    }
    ...
    // put topic nodes in a Set
    setNodes = new Set(ind);
    console.log(topics);

    // loop through all the data in tweets_topics.json and only add node index to
    // ind if tweet falls between date range selection, is created in the selected
    // state, and the sentiment is within the sentiment selection range 
    for (i = 0; i < data.length; i++) {
        if ((data[i].datejs >= startDate ) && (data[i].datejs <= endDate) && 
            (state_ind.includes(data[i].state)) && (data[i].sentiment >= senti_start) && 
            (data[i].sentiment < senti_end)) {
          ind.push(i);
        }
    }   

    // goes through each link from the from the graph json file, and only adds 
    // links with nodes that are in ind to the final filtered link Array list
    // checks and make sure that both target and source are in ind
    var j = 0;
    for (j = 0; j < links_g.length; j++) {
      if ((ind.includes(links_g[j].target)) && (ind.includes(links_g[j].source))) {
          linksArray.push(links_g[j]);
          setNodes.add(links_g[j].target);
      }
      else if ((ind.includes(links_g[j].target.id)) && (ind.includes(links_g[j].source.id))) {
        linksArray.push(links_g[j]);
        setNodes.add(links_g[j].target.id);
      }
    }

    // adds non topic nodes after filtering to the final nodes array from the set
    // generated previously
    for (i = 0; i < nodes_g.length; i++) {
      if (setNodes.has(nodes_g[i].id)) {
          nodesArray.push(nodes_g[i]);
      }
  } 
    
    // update graph variable that is used to create visualization wheel later in 
    // the code with nodes and links based on user filter.
...
/**  Adding a Javascript Date object to each tweet in data value read 
     from tweets_topics.csv */
function addDate() {
    var i;
    var datejs = new Date();
    for (i = 0; i < data.length; i++) {
        datejs = new Date(data[i].date);
        data[i]["datejs"] = datejs;
    }
}
...
function draw_wheel(){
    ...
    // load default filtering options
    start = "2020-12-01";
    end = "2021-07-01";
    topics = "All Topics";
    state = "All States";
    ...
    *****************************************************************/
    ...
    // COVIZVAX filtering options code (start): 
    // ----------------------------------------------------------------------
    // update function which is called after changing of any filter
    // it updates the nodes and links in the visualization wheel
    function update() {
      parseData();
      ticked();
      simulation
      ...
      simulation.force('link')
        ...
        // create topic filter
        d3.select(".topicSelect")
          .selectAll('topicOptions')
           .data(allGroup)
          .enter()
          .append('option')
        ...
        d3.select('.topicSelect').property('value', allGroup[10]);
        // d3.select('#selectButton').property('value', '2021-06-01');
        // TODO: visualizing a smaller set of data as default and then
        // expanding to a larger set of data causes the graph to display an inner
        // circle of nodes which should not have happened      
      
        // this code tracks user selection and calls update function to 
        // update the links and nodes in the graph based on filter change
        d3.select(".topicSelect").on("change", function(d) {
            // recover the option that has been chosen
            topics = d3.select(this).property("value");
            update();
        })
        ...
        $('input[name="daterange"]').daterangepicker({
          opens: 'left'
        }, function(startV, endV, label) {
          // updates visualization wheel after obtaining new selection
          start = startV;
          end = endV;
          update();
        });
        ...
        // set default value for drop down
        d3.select('.stateSelect').property('value', stateGroup[0]);     
      
        // this code tracks user selection and calls update function to 
        // update the links and nodes in the graph
      ...
      d3.select(".sentiSelect")
        .selectAll('sentiOptions')
         .data(sentiGroup)
        .enter()
        .append('option')
        .text(function (d) { return d; }) // text showed in the menu
      ...
      // set default value for drop down
      d3.select('.sentiSelect').property('value', sentiGroup[3]);
  
    
      // this code tracks user selection and calls update function to 
      // update the links and nodes in the graph
      d3.select(".sentiSelect").on("change", function(d) {
          // recover the option that has been chosen
          senti = d3.select(this).property("value");
          update();
      })

    // ------------------------------------------------------------------------
    // COVIZVAX filtering options code (end) 
  ...
  function ticked(){
	 ...
	 if(normal){
      ...
      // TODO: color definition
      context.beginPath();
      graph.nodes.forEach(drawNode_tweet_A);
      ...
      context.beginPath();
      graph.nodes.forEach(drawNode_tweet_B);
      ...
      context.beginPath();
      graph.nodes.forEach(drawNode_tweet_C);
...
function map_plot_fc(){
  ...
  function render() {
    ...
    data.forEach(function(d) {
      var ready = map_view.project(new mapboxgl.LngLat(d.longitude,d.latitude));
